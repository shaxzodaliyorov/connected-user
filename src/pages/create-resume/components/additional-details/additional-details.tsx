...
export const AdditionalDetails: FC<Props> = ({form}) => {
  const [getAllNationality, {data: {data: nationalities = []} = {}, isFetching: nationalityFetching}] =
  ...
  const fetchAllNationality = async () => {
    await handleRequest({
      request: async () => {
        const response = await getAllNationality({search: form.watch('nationality')}).unwrap()
        return response
      },
    })
  }
  ...
  return (
    <div>
      ...
      <div className="pb-6">
        <h1 className="text-[#0b0b0b] text-xl font-medium font-['Inter Display'] leading-snug">Additional Details</h1>
      </div>
      <div className="grid gap-6 grid-cols-2">
        <Controller
          ...
          render={({field}) => (
            <SearchInput
              ...
              options={nationalities?.map(item => ({label: t(item.name), value: item.name}))}
        ...
        <div className="grid gap-6 grid-cols-2">
          <Controller
            ...
            render={({field}) => (
              <Select
                options={JOB_TYPES.map(item => ({label: t(item.value), value: item.value}))}
          ...
          <Controller
            ...
            render={({field}) => (
              <Select
                options={EXPERIENCE_LEVELS.map(item => ({label: t(item.label), value: item.value}))}
