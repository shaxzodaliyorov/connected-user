...
export const JobDetails = () => {
  ...
  const { id } = useParams();
  const { data: { data: job } = {}, isLoading } = useGetPublicJobQuery({
    id: id!,
  });
    ...
    await handleRequest({
      request: async () => {
        const response = await saveJob({
          ...
          type: saveJobIds.includes(job?._id as string)
            ? "unfavorite"
            : "favorite",
        });
        return response;
      ...
      onSuccess: () => {
        toast({
          title: saveJobIds.includes(job?._id as string)
            ? t("Job Unsaved")
            : t("Job Saved"),
          variant: "default",
        });
          ...
          <div className="w-2/3 flex flex-col gap-4 ">
            <div className=" w-full p-[30px] bg-white rounded-[15px]">
              <div className=" flex justify-between gap-4">
                <h1 className="text-2xl font-medium leading-[29.04px] text-left">
                  {job?.job_title}
                </h1>
                <div className="flex gap-6 items-center">
                  <button onClick={onSaveAndUnsaved}>
                    {saveJobIds.includes(job?._id as string) ? (
                      <FillLike />
                    ) : (
                      <Like />
                    )}
              ...
              <div className="flex justify-between gap-10">
                <div className="w-2/3 flex flex-col">
                  <div className="flex justify-between items-center">
                    <h5 className="text-sm font-medium leading-[22.4px] text-left text-[#7D7D7D]">
                      Type of work
                    </h5>
            ...
            <div className="w-full p-[30px] bg-white rounded-[15px]">
              <h1 className="text-xl font-medium leading-[24.2px] text-left text-[#0C0C0C]">
              ...
              <div className="pt-9 flex gap-3">
                <img
                  className="w-[435px] border h-[254px] rounded-lg"
                  src={job?.company?.images[0]}
                  alt=""
                />
              ...
              <div className="pt-9">
                ...
                <div className="grid pt-4 grid-cols-1 xl:grid-cols-2 gap-y-[25px] ">
                  {BENEFIT?.filter((item) =>
                    job?.benefit?.includes(item.name)
                  )?.map((item, index) => (
                    <label
                      key={index}
                    ...
                    >
                      <p className="text-[#0b0b0b] flex items-center gap-x-2.5 text-base font-normal font-['Inter Display'] leading-relaxed">
                        <span className="text-[#1289F8]">{item.icon}</span>{" "}
                        {item.name}
                      </p>
                    </label>
                  ))}
              ...
              <div className="pt-9">
                ...
                <div className="grid pt-4 grid-cols-1 xl:grid-cols-2 gap-y-[25px] ">
                  {QUALIFICATIONS?.filter((item) =>
                    job?.qualification?.includes(item.name)
                  )?.map((item, index) => (
                    <label
                      key={index}
                      className="flex select-none items-center  gap-x-3"
              ...
              <div className="pt-9">
                ...
                <h4 className="text-[16px] font-semibold pt-4 leading-6 text-left">
                  Address
                </h4>
                ...
                <div className="pt-4 flex flex-col gap-y-1">
                  <h4 className="text-[16px] font-semibold leading-6 text-left">
                    Contact
                  </h4>
                  <p className="text-base font-normal leading-6 text-left">
            ...
            <div className="w-full">
              <div className="py-10">
                <h3 className="text-xl font-semibold leading-[24.2px] text-left">
                  Similar vacancies
                </h3>
              </div>
              <div className="w-full  flex flex-col gap-4">
                {jobs
                  ...
                  .map((jobItem, index) => (
                    <JobItem key={index} job={jobItem as Job} />
                  ))}
                {next_page && (
                  <Button
