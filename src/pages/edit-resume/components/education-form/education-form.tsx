...
import {useLazyGetAllCountryQuery, useLazyGetAllUniversityQuery} from '@/features/info'
...
export const EducationForm: FC<Props> = ({form, fields, index, remove}) => {
  const [getUniversity, {data: {data: universities = []} = {}, isFetching}] = useLazyGetAllUniversityQuery()
  const [getCountry, {data: {data: countries = []} = {}, isFetching: isFetchCountry}] = useLazyGetAllCountryQuery()
  const getFieldName = useCallback((name: string) => `education.${index}.${name}`, [index])
  ...
  const fetchUniversities = async () => {
    await handleRequest({
      request: async () => {
        const response = await getUniversity({search: form.watch(getFieldName('university'))})
        return response
      },
    })
  }
  ...
  const fetchCountry = async () => {
    await handleRequest({
      request: async () => {
        const response = await getCountry({search: form.watch(getFieldName('region'))})
        return response
      },
    })
  }
  ...
  useEffect(() => {
    if (form.watch(getFieldName('university')) && form.watch(getFieldName('university')).length > 2) {
      fetchUniversities()
    }
  }, [form.watch(getFieldName('university'))])

  useEffect(() => {
    if (form.watch(getFieldName('region')) && form.watch(getFieldName('region')).length > 2) {
      fetchCountry()
    }
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-[#0b0b0b] text-2xl font-medium font-['Inter Display'] leading-relaxed">Add Education</h1>
        {fields.length > 1 && (
          <button
            onClick={remove}
      ...
      <div className="grid pt-8 gap-6 grid-cols-2">
        ...
        <Controller
          ...
          render={({field}) => (
            <SearchInput
              options={universities.map(item => ({label: item.en, value: item.en}))}
              errorMessage={
                ((
                  form.formState.errors?.education?.[
                    index as unknown as keyof typeof form.formState.errors.education
        ...
        <div className="grid gap-x-[30px] grid-cols-2">
          <Controller
            ...
            render={({field}) => (
              <Select
                options={getLastYears().map(item => ({label: item.toString(), value: item.toString()})) || []}
                errorMessage={
                  ((
                    form.formState.errors?.education?.[
                      index as unknown as keyof typeof form.formState.errors.education
